(function () {
  ...
  2: [function (require, module, exports) {
     ...
     * @LastEditTime: 2021-10-23 14:46:11
    ...
    exports.loginStatus =
      async function (cookieToken, axios) {
        const res = await axios({
          url: '/login/status',
          method: 'post',
          data: {
  ...
  3: [function (require, module, exports) {
      ...
      if (isTelephone(cellphone.value)) {
        ...
        cellphone_status.then(res => {
          ...
          if (cellphone_exist == '1') {
            ...
            getCaptcha.style.opacity = 0.5;
            getCaptcha.style.pointerEvents = "none";
            // getCaptcha.style.cursor = "initial";
            var time = 60;
            getCaptcha.innerHTML = time + "秒后重试";
            var count = setInterval(function countDown() {
              ...
              if (time == 0) {
                clearInterval(count);
                getCaptcha.style = '';
                getCaptcha.style.pointerEvents = "initial";
    ...
    login.onsubmit = function () {
      ...
      if (cellphone == preCellphone) { //cellphone == preCellphone
        var token = user_info_API.loginByPhone(cellphone, captcha, axios);
        // console.log(token);
        token.then(res => {
          cookieToken = res;
          if (cookieToken != '-1') { //登录成功
            window.localStorage.setItem("cookie", cookieToken);
            // var login_status = user_info_API.loginStatus(cookieToken, axios);  //登录状态
            var user_info_data = user_info_API.getUserInfo(cookieToken, axios);
            user_info_data.then(res => {
              user_info = res;
              window.localStorage.setItem("userId", user_info.account.id);
              // console.log(res);
            })
            // console.log(cellphone);
            isExistCellphone(cellphone);
          } else {
            document.getElementById("captcha_error").style.display = "block";
          }
        })
    ...
    // isExistCellphone('18350803960');

    function isExistCellphone(cellphone) {
      var user_cellphone = {
        cellphone: cellphone
      }
      var params = JSON.stringify(user_cellphone);
      ...
      xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (user_info['info'] == 1) { //注册过
            // console.log(user_info);
            var user_data = JSON.stringify(user_info);
            window.localStorage.setItem("nowUser_Uid", user_info['uid']);
            window.localStorage.setItem("user" + user_info['uid'], user_data);
            alert("登录成功");
            location.reload();
          } else if (user_info['info'] == -1) {
            // console.log(2);
            registerByCellphone(cellphone);
          }
        }
      }
      xmlhttp.open('GET', 'http://localhost:3000/get_user_info_by_cellphone.php?data=' + params, true);
      xmlhttp.send();
    }

    async function registerByCellphone(cellphone) {
      var register_info = {
        "user_name": cellphone,
        "user_password": '123456', //默认密码
        "register_method": '手机号注册'
      };
      var params = JSON.stringify(register_info);
      var xmlhttp = new XMLHttpRequest();
      xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          // console.log(this.responseText);
      ...
      xmlhttp.open('GET', 'http://localhost:3000/add_user_info.php?data=' + params, true)
      xmlhttp.send();
    }

    function isTelephone(telephone) {
      if (telephone.length == 11) {
        var status = /^(136|135|183|133|149|153|173|177|180|181|189|199|130|131|132|145|155|156|166|171|175|176|185|186|166|134|137|138|139|147|150|151|152|157|158|159|182|184|187|188|198)\d+$/.test(telephone);
        // console.log(status);
        return status;
      }
      return false;
    ...
    //     }
    //   })
    //   qr_img.src = res2.data.data.qrimg;
    ...
    //       // user_info_API.getUserInfo(cookieToken, axios);
    ...
    "./get_user_info_API": 2,
    "./user_cellphone_API": 4,
    "npm/node_modules/axios": 5
  }],
  4: [function (require, module, exports) {
     ...
     * @LastEditTime: 2021-10-23 14:37:40
  ...
  6: [function (require, module, exports) {
    ...
    "./../helpers/buildURL": 23,
    "./../helpers/cookies": 25,
    "./../helpers/isURLSameOrigin": 28,
    "./../helpers/parseHeaders": 30,
    "./../utils": 33
  }],
  ...
  20: [function (require, module, exports) {
    ...
    "./adapters/http": 6,
    "./adapters/xhr": 6,
    "./core/enhanceError": 16,
    "./helpers/normalizeHeaderName": 29,
    "./utils": 33,
    "_process": 1
  }],
