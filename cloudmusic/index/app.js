(function () {
  ...
  2: [function (require, module, exports) {
     ...
     * @LastEditTime: 2021-10-21 20:47:15
      ...
      if (isTelephone(cellphone)) { //要传入字符串
        ...
        var count = setInterval(function countDown() {
          ...
          if (time == 0) {
            ...
            captcha.style = '';
            captcha.style.pointerEvents = "initial";
    ...
    update_telephone.onsubmit = function () {
      ...
      if (cellphone.length > 0 && captcha.length > 0) {
        ...
        bindStatus.then(res => {
          ...
          if (bind == '1') { //验证码正确
            ...
            document.getElementById("captcha_error").value = "验证码错误";
            document.getElementById("captcha_error").style.display = "block";
            var cellphone1 = document.getElementById("user_cellphone");
            cellphone1.value = '';
            var captcha1 = document.getElementById("cellphone_captcha");
            captcha1.value = '';
            cellphone.focus();
          }
        })
      } else if (cellphone.length == 0) {
        document.getElementById("captcha_error").innerHTML = "手机号不能为空";
        document.getElementById("captcha_error").style.display = "block";
      } else if (captcha.length == 0) {
        document.getElementById("captcha_error").innerHTML = "验证码不能为空";
        document.getElementById("captcha_error").style.display = "block";
      }
      return false;
    }
    ...
    async function update_cellphone(cellphone) {
      ...
      xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
          ...
          if (info == "更改成功") {
            // location.reload();
            var now_user_info = JSON.parse(window.localStorage.getItem("user" + uid));
            now_user_info['ucellphone'] = cellphone;
            window.localStorage.setItem("user" + uid, JSON.stringify(now_user_info));
            if (cellphone == '') {
              cellphone = "无";
            ...
            document.getElementById("user_telephone_info").innerHTML = cellphone;
          }
          var update_window = document.getElementById('update_telephone_window');
    ...
    function isTelephone(telephone) {
      if (telephone.length == 11) {
        var status = /^(136|135|183|133|149|153|173|177|180|181|189|199|130|131|132|145|155|156|166|171|175|176|185|186|166|134|137|138|139|147|150|151|152|157|158|159|172|178|182|184|187|188|198)\d+$/.test(telephone);
        // console.log(status);
        return status;
      }
      return false;
  ...
  3: [function (require, module, exports) {
     ...
     * @LastEditTime: 2021-10-21 20:46:20
